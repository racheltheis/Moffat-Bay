...
PHONE_RE = re.compile(r"^\+?1?\s*\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}$")  # US phone number regex
...
def login_post():
    ...
    try:
        ...
        # Handle invalid user or password safely
        if not user:
        ...
        try:
            ...
            # Handle invalid salt or corrupted hash gracefully
            flash("Incorrect password, please try again.", "error")
            return redirect(url_for("auth.login_get"))
        ...
        flash("Welcome back Moffat Bay Lodge!", "success")
